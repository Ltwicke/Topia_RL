...
def _mlp(in_d: int, hid_d: int, out_d: int, depth: int = 2) -> nn.Sequential:
    ...
    Minimum depth = 1 → [Linear(in→hid), ReLU, LayerNorm(hid), Linear(hid→out)]
    depth = 2       → [Linear(in→hid), ReLU, Linear(hid→hid), ReLU, LayerNorm(hid), Linear(hid→out)]
    etc.

    LayerNorm is applied once, directly before the final linear layer, to
    stabilise gradient magnitudes across all decision heads.
    """
    assert depth >= 1, "depth must be >= 1"
    layers: list[nn.Module] = [nn.Linear(in_d, hid_d), nn.ReLU()]
    for _ in range(depth - 1):
        layers += [nn.Linear(hid_d, hid_d), nn.ReLU()]
    layers.append(nn.LayerNorm(hid_d))          # ← stabilise pre-output activations
...
class PolicyNetwork(nn.Module):
    ...
    # ── compute_values_batch : critic-only forward pass ───────────────────────

    def compute_values_batch(self, obs_snaps: list) -> torch.Tensor:
        """
        Run the critic on a list of stored obs_snaps without computing any
        policy quantities.  Used by the trainer to recompute fresh value
        estimates for GAE re-calculation at the start of each PPO epoch.

        Parameters
        ----------
        obs_snaps : list of dicts with keys 'graph', 'Nx', 'Ny'

        Returns
        -------
        values : Tensor (B,) on model device
        """
        graphs      = [snap['graph']          for snap in obs_snaps]
        board_sizes = [(snap['Nx'], snap['Ny']) for snap in obs_snaps]
        _, global_embs = self._encode_batch(graphs, board_sizes)
        return self.critic_head(global_embs).squeeze(-1)   # (B,)
