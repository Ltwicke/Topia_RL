...
### Hidden tile estimator network design patches

- Some changes are necessary for the hidden tile estimator:
- First, it should not produce tile-feature dimensional outputs; Why is that? By the design of the game, in hidden tiles can lie only enemy units, enemy controlled area and enemy cities. Therefore, it is not necessary to, for example, predict if there is an own controlled tile in the hidden tiles.
- Reduce the dimensions in the following way: collapse the tile features to only the opponent features (aka opponent controlls tile can now be a sigmoid where 0 indicates no one controlls tile and 1 indicates certainty that opponent controlls tile).
- Every enemy unit can only be in turn state idle when the player is doing its turn and therefore, only the unit types are interesting. Remove unit turn state entirely for the hidden estimation and only predict the unit types (or predict no unit type if the tile is not occupied)
- Collapse the city features to a one-hot encoding based on the level of the city only (ignore choices) up to level 8+. Additionally, one option must remain for village and one option for None (just empty tile). 
- During the loss compute, you can algorithmically transform the full tile feature vector to the reduced form for the hidden tile estimator to calculate the loss.
- Also cascade these changes to the hidden tile estimation rendering in the rendering function.
